...
subroutine diff(N, D, X, Lx, Ly, Lz, der)
  ...
  if (D == 3) then
    ...
    !$OMP PRIVATE (i, j, LLxax, LLyay, LLzaz, dist, tmp)             &
    !$OMP SHARED (N, LLx, LLy, LLz, LLxx_inv2, LLyy_inv2, LLzz_inv2, &
    !$OMP         LLx_inv, LLy_inv, LLz_inv, X, df)
    !$OMP DO
    do j = 1, N-1
      do i = j+1, N
          ...
          dist = LLxx_inv2 * (1.d0 - dcos(LLxax)) &
               + LLyy_inv2 * (1.d0 - dcos(LLyay)) &
               + LLzz_inv2 * (1.d0 - dcos(LLzaz))

!          if(dabs(dist) .gt. 1d-8) then
            tmp = 1.d0 / (dist * dsqrt(dist))
            df(1,i,j) = tmp * dsin(LLxax) * LLx_inv
            df(2,i,j) = tmp * dsin(LLyay) * LLy_inv
            df(3,i,j) = tmp * dsin(LLzaz) * LLz_inv
!          else
!            df(1:3,i,j) = 0.d0
!          endif
      enddo
    ...
    !$OMP PARALLEL DEFAULT(NONE) &
    !$OMP PRIVATE(j, i) SHARED(N, df)
    !$OMP DO
    do j = 1, N
      df(1:3,j,j) = 0.d0
      do i = j+1, N
        df(1:3,j,i) = -df(1:3,i,j)
      enddo
    enddo
    !$OMP END DO
    !$OMP END PARALLEL
  ...
  if (D == 3) then
    ...
    do j = 1, N-1
      do i = j+1, N
        ...
        H(1:3,1,j,i) = H(1:3,1,i,j)
        H(2:3,2,j,i) = H(2:3,2,i,j)
        H(3,3,j,i) = H(3,3,i,j)
      end do 
    end do 
    !$OMP END DO
